      ...
      {open && (
        <div className="fixed left-0 right-0 top-18 z-40 border-b border-(--border-subtle) bg-(--bg-header-mobile) p-5 backdrop-blur-xl transition-colors duration-300 md:hidden">
          <div className="flex flex-col gap-2">
            ...
            <a href="/dashboard" onClick={closeMenu} className="rounded-xl px-4 py-3 text-(--text-secondary) transition-colors hover:bg-(--text-primary)/5 hover:text-(--text-primary)">
              Dashboard
            </a>

            <a href="/login" onClick={closeMenu} className="rounded-xl px-4 py-3 text-(--text-secondary) transition-colors hover:bg-(--text-primary)/5 hover:text-(--text-primary)">
              Login
            </a>
